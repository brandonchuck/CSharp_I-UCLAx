...
/// <summary>
/// Creates the rooms, transitions, and items for this adventure game and connects them. INHERITS FROM GAME CLASS
/// </summary>
public class InheritorGame : Game
{
    ...
    // -------------------- CREATING ROOM OBJECTS + Initializations --------------------

    // outside the house (has no items)
    Room outside = new Room { RoomDescription = "You decide to take what you've already found and leave. Something about this place unnerves you, and you never return.", Items = new List<Item>(), FinalRoom = true };


    // entrance hall (has no items)
    Room entranceHall = new Room { RoomDescription = "You are in the entrance hall. There is a door leading further into the house to the north, and an exit to the south.", Items = new List<Item>() };
    ...
    Room cellar = new Room
    {
        ...
        Items = new List<Item>{
        new Item { Name = "wine", Description = "A bottle of fine wine.", PointValue = 50 }, // initializing values for Item object; can have multiple items in the "Items" list
    }
    };
    ...
    // Describes possible current room, connections from that room, and direction you could go in
    private void SetupRooms()
    {
        ...
        {
            {SOUTH, entranceHall}, // south from living room is entranceHall
            {NORTH, fancyBedroom},
            {EAST, kitchen},
            {WEST, paintingRoom},
            {DOWN, cellar},   // can move down to cellar from living room
        };

        // fancy bedroom connections
        fancyBedroom.Transitions = new Dictionary<string, Room> 
        {
            {SOUTH, livingRoom }, // living room is only room south of Fancy Bedroom
        };

        // kitchen connections
        kitchen.Transitions = new Dictionary<string, Room>
        {
            { WEST, livingRoom },  // living room is only room to west of Kitchen
        };

        // painting room connections
    ...
    // -------------------- CONSTRUCTOR for InheritorGame() Class --------------------
